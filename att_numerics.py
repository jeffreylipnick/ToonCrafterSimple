...
x, ctx = torch.randn(16, 512, 40, 64, device="cuda"), torch.randn(
    16, 512, 2, 40, 64, device="cuda"
)
with torch.cuda.amp.autocast(dtype=torch.float16):
    o0, o1 = c0(x.clone(), ctx), c1(x.clone(), ctx)
...
d = {
    k: v[..., None, None] if k[0] in "qkvp" and k[-1] == "t" else v
    for k, v in ab0.state_dict().items()
}
