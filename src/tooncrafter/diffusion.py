...
class DDIMWrapper(DDPM):
    ...
    def forward(
        ...
        x: Tensor,
        t: Tensor,
        c_concat: list[Tensor] | None = None,
        c_crossattn: list[Tensor] | None = None,
        **kwargs: UNetKwargs,
    ):
    ...
    def sample(
        ...
        S: int,
        batched_noise_shape: list[int],
        eta: float = 0.0,
        timestep_spacing="uniform",  # scheduler kwargs
        x_T=None,
        verbose=True,
        # unused blending kwargs
        mask=None,
        x0=None,
        clean_cond: bool = False,
        # p_sample kwargs
        conditioning: dict | None = None,
        unconditional_conditioning: dict = None,
        ...
        **kwargs: UNetKwargs,
        ...
        if conditioning is not None:
            if isinstance(conditioning, dict):
                ...
                cond_bs = (
                    any_cond_tensor[0]
                    if isinstance(any_cond_tensor, list)
                    else any_cond_tensor
                ).size(0)
            else:
                cond_bs = conditioning.size(0)
            if cond_bs != batch_size:
                print(
                    f"Warning: Got {cond_bs} conditionings but batch-size is {batch_size}"
                )
        ...
        for i, step in enumerate(iterator):
            ...
            if mask is not None:
                ...
                img_orig = (
                    x0 if clean_cond else self.q_sample(x0, ts)
    ...
    def p_sample_ddim(
        ...
        **kwargs: UNetKwargs,
        ...
        e_t = (
            self.predict_eps_from_z_and_v(x, t, model_output)
        ...
        pred_x0 = (
            (x - sqrt_one_minus_at * e_t) / a_t.sqrt()
            if self.parameterization != "v"
            else self.predict_start_from_z_and_v(x, t, model_output)
        )
