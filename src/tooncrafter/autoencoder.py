        ...
        **ignore_kwargs,
        ...
        for i_level in range(self.num_resolutions):
            ...
            block = nn.ModuleList(
                [
                    ResnetBlock(
                        block_out if i_block else block_in,
                        block_out,
        ...
        self.mid = nn.ModuleDict(
            dict(
                block_1=ResnetBlock(
                    block_in, block_in, temb_channels=self.temb_ch, dropout=dropout
                ),
...
class Decoder(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        make_resblock_cls = partial(
            self._make_resblock(), temb_channels=self.temb_ch, dropout=dropout
        )
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            block_in = block_out
            attn_refinement = (
                make_attn(block_in, attn_type="memory-efficient-cross-attn-fusion")
                if i_level in self.attn_level
                else Combiner(block_in)
            )
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.out_layers = nn.Sequential(
            ...
            conv_nd(
                dims, self.out_channels, self.out_channels, kernel_size, padding=padding
            ),
        )
...
class AE3DConv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, video_kernel_size=3, *args, **kwargs):
        ...
        self.time_mix_conv = nn.Conv3d(
            out_channels, out_channels, kernel_size=video_kernel_size, padding=padding
        )
...
class VideoDecoder(Decoder):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            time_mode in self.available_time_modes
    ...
    def _make_resblock(self) -> Callable:
        return partialclass(
            VideoResBlock, video_kernel_size=self.video_kernel_size, alpha=self.alpha
        )
