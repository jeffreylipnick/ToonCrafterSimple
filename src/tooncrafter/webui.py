...
i2v_examples_interp_512 = [
    [
        "https://raw.githubusercontent.com/ToonCrafter/ToonCrafter/main/assets/Japan_v2_2_062266_s2_frame1.png",
        "an anime scene",
        "https://raw.githubusercontent.com/ToonCrafter/ToonCrafter/main/assets/Japan_v2_2_062266_s2_frame3.png",
        50,
        7.5,
        1.0,
        10,
...
def dynamicrafter_demo(result_dir="./tmp/", res=512, compile: bool = False):
    ...
    image2video = Image2Video(
        "", result_dir, resolution=resolution, fp16=True, compile=compile
    )
        ...
        with gr.Tab(label="ToonCrafter_320x512"):
            with gr.Column():
                with gr.Row():
                    with gr.Column():
                        with gr.Row():
                            i2v_input_image = gr.Image(
                                label="Input Image1", elem_id="input_img"
                            )
                        with gr.Row():
                    ...
                    with gr.Column():
                        with gr.Row():
                            i2v_input_image2 = gr.Image(
                                label="Input Image2", elem_id="input_img2"
                            )
                        with gr.Row():
...
def main():
    ...
    dynamicrafter_iface = dynamicrafter_demo(
        args.result_dir, compile=bool(args.compile)
    )
    dynamicrafter_iface.queue(max_size=12)
    dynamicrafter_iface.launch(
        server_name=args.hostname, server_port=args.port, max_threads=1
    )
