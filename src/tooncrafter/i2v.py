...
from tooncrafter.extras import (
    seed_everything,
...
def save_videos(
    batch_tensors: torch.Tensor, savedir: Path, filenames: list[str], fps: int = 10
):
    # b,samples,c,t,h,w
...
@timed
def spawn_model(
    config_path: str, ckpt_path: str = "model.safetensors"
) -> ToonCrafterModel:
    """
    ...
    if config_path:
        ...
        with resources.as_file(
            resources.files("tooncrafter").joinpath("new.yaml")
        ) as p:
...
def batch_ddim_sampling(
    ...
    *,
    ...
    )  # TODO: possible to avoid repeated work here?
...
def get_latent_z(
    model: ToonCrafterModel, videos: torch.Tensor, *, return_hidden_states=True
...
class Image2Video:
    def __init__(
        ...
        self.model: ToonCrafterModel = spawn_model(
            config, ckpt_path=self.download_model(fp16)
        )
        self.save_fps = 8
        ...
        if compile:
            self.model.model.diffusion_model = torch.compile(
                self.model.model.diffusion_model, fullgraph=True
            )
    ...
    def to_videos(self, image: np.ndarray, frames: int):  # image is hw3
        ...
        videos = repeat(
            img_resized, "c h w -> 1 c repeat h w", repeat=frames // 2
        )  # b3thw
        return videos, img

    @timed
    def prepare_cond(
        self, prompt: str, image1: np.ndarray, image2: np.ndarray, frames: int, fs: int
    ):
        model = self.model
        ...
        return hs, Conditioning(
            [imtext_cond],
            [img_tensor_repeat],
            torch.tensor([fs], dtype=torch.long, device=model.device),
        )
    ...
    def get_image(
        ...
        prompt_str = (
            prompt.replace("/", "_slash_").replace(" ", "_")[:40] or "empty_prompt"
        )
        save_videos(
    ...
    def download_model(self, fp16: bool = False) -> str:
        if fp16:
            ...
            # )
        return hf_hub_download(
            "Doubiiu/ToonCrafter", "model.safetensors", revision="refs/pr/4"
        )
